...
public class Logger
{
...
#else
        LoggerHelper.Instance.LogToCustomer(s, p);
#endif
    }
    ...
    {
        // if (!string.IsNullOrEmpty(postData) && !string.IsNullOrEmpty(AssetBundleManager.REPORT_ERROR_URL))
        // {
        ...
        //     m_webClient.UploadStringAsync(new Uri(AssetBundleManager.REPORT_ERROR_URL), "POST", postData);
        // }
    }
    ...
    private static void DealWithReportError()
    {
        ...
        if (errorCount > 0)
        {
            ...
            counter = 0;
            sb.Length = 0;
            // if (!string.IsNullOrEmpty(AssetBundleManager.REPORT_ERROR_URL))
            // {
